...
- SOXL 20분할 기본 설정과 복리/단리, 기간, 원금, 매도 기준 변경
...
- version2 전용 Yahoo 실제 OHLCV 다운로드(분할 반영·배당 미보정)

브라우저 왼쪽 설정 패널 맨 아래의 `전체 가격 데이터` 영역에서 `전체 데이터 갱신`을 누르면 세 종목의 전체 가용 이력을 오늘 날짜까지 한 번에 갱신합니다. 이 작업은 위의 백테스트 분석 기간 설정과 독립적입니다. 2.4.0 이전에 받은 데이터는 배당까지 소급 조정된 가격이므로 업그레이드 후 한 번 다시 갱신해야 합니다.
