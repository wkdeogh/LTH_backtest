...
- 선택 기간의 조정 OHLC 캔들·거래량 차트와 60/120/252거래일 구간 이동
...
일반 백테스트를 실행한 뒤 `캔들 차트` 탭을 열면 설정한 기간의 분할·배당 조정 OHLC와 거래량을 확인할 수 있습니다. 기본값은 마지막 120거래일이며 60일, 252일, 전체 기간으로 바꾸거나 이전·다음 구간을 이동할 수 있습니다. 캔들 위에 포인터를 올리면 날짜, 시가, 고가, 저가, 종가, 전일 대비 등락률, 거래량을 표시합니다.
