...
- 선택 기간의 조정 OHLC 캔들·거래량 차트, 국내식 상승 빨강·하락 파랑 표시와 60/120/252거래일 구간 이동
...
브라우저 왼쪽 설정 패널 맨 아래의 `전체 가격 데이터` 영역에서 `전체 데이터 갱신`을 누르면 세 종목의 전체 가용 이력을 오늘 날짜까지 한 번에 갱신합니다. 이 작업은 위의 백테스트 분석 기간 설정과 독립적입니다.
...
일반 백테스트를 실행한 뒤 `캔들 차트` 탭을 열면 설정한 기간의 분할·배당 조정 OHLC와 거래량을 확인할 수 있습니다. 굵은 몸통은 시가와 종가, 얇은 꼬리는 고가와 저가를 나타내며 상승봉은 빨강, 하락봉은 파랑입니다. 기본값은 마지막 120거래일이며 60일, 252일, 전체 기간으로 바꾸거나 이전·다음 구간을 이동할 수 있습니다. 캔들 위에 포인터를 올리면 날짜, 시가, 고가, 저가, 종가, 시가·전일 대비 등락률, 거래량을 표시합니다.
