...
class WebConfigurationTests(unittest.TestCase):
    ...
    def test_six_strategy_comparison_random_cards_and_sweep_visuals_are_integrated(self) -> None:
        ...
        self.assertIn('id="uniformSamplingOption" data-analysis-modes="lth_v4 compare"', html)
        self.assertIn('name="uniform_start_sampling" id="uniformStartSampling"', html)
        self.assertIn("균등 시작일 샘플링", html)
        self.assertIn('class="random-layout" id="randomLayout"', html)
        ...
        self.assertIn("function syncUniformSamplingControls(mode = selectedAnalysisMode())", javascript)
        self.assertIn('uniform_start_sampling: raw.get("uniform_start_sampling") === "on"', javascript)
        self.assertIn('form.elements.max_days.disabled = active', javascript)
        self.assertIn('form.elements.seed.disabled = active', javascript)
        ...
        self.assertIn("function renderLthRandom(result)", javascript)
        self.assertIn('result?.result_type === "lth_random_comparison"', javascript)
        self.assertIn("function strategyRandomDistributionStats(result = app.randomResult)", javascript)
        self.assertIn("function drawRandomDistributionChart(canvas, metricKey)", javascript)
        self.assertIn("function drawRandomRankProbabilityChart()", javascript)
        self.assertIn('id="randomReturnDistributionChart"', javascript)
        self.assertIn('id="randomMddDistributionChart"', javascript)
        self.assertIn('id="randomRankProbabilityChart"', javascript)
        ...
        self.assertIn(".random-layout.strategy-mode { grid-template-columns: minmax(0,1fr); }", stylesheet)
        self.assertIn(".random-layout.strategy-mode #randomLthOptions", stylesheet)
        self.assertIn(".random-analysis-stack { display: grid; grid-template-columns: minmax(0,1fr)", stylesheet)
        self.assertIn(".random-insight-strip", stylesheet)
        self.assertIn(".random-sampling-option:has(input:checked)", stylesheet)
